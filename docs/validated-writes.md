...
| reMarkable (.rmdoc) | reMarkable desktop (Mac), File → Import | rm-writer-roundtrip.rmdoc + saber-to-rm.rmdoc | Round 1 **FAIL** — "Unable to import. No such file or directory". Rebuilt the container as a field-for-field replica of a real desktop-cache document (full `.content` incl. `cPages` idx/lastOpened/original/uuids CRDT author table, full `.metadata`, `.local` member). Round 2 **BLOCKED** — the File → Import picker greys out `.rmdoc` entirely (accepts pdf/epub only); next attempt: drag-and-drop onto the library window, else the rmapi lane. No .rmdoc ground truth yet (case B ask). | PASS (`test_remarkable_writer.py`) | maintainer (app) + Claude (cache-replica fix) | 2026-07-09 |
| GoodNotes (.goodnotes) | GoodNotes Mac | goodnotes-roundtrip + rm-to-goodnotes | Round 1 **FAIL** — would not import (no dialog detail). Container was missing 6 of the 10 members the app writes; writer now emits the full observed set and replays the per-page metadata record byte-verbatim on round-trips. Round 2 **FAIL** — fixture byte-copy imports fine (app-version control PASS) but ours fail with "missing document id": the document UUID lives only in `index.events.pb`, which we wrote empty. Writer now synthesizes a document-created event (doc UUID + title + first-page ref) and an attachment-added event, mirroring the export's first two records. Awaiting round 3. | PASS (`test_goodnotes_writer.py`) | maintainer (app) + Claude (member-set + events fix) | 2026-07-09 |
| Notability (.ntb) | Notability Mac | ntb-roundtrip + rm-to-notability | Round 1 **FAIL** — "FlatBuffers.FlatbuffersErrors error 0". Structural diff vs the app's buffer: our builder packed fields in the opposite order from the official FlatBuffers builders, didn't dedup vtables, misaligned the root 16-byte struct and zeroed it. All fixed. Round 2: **rm-to-notability PASS** (imports and renders — the rebuilt builder is accepted); fixture byte-copy PASS; the two files carrying the fixture's original note UUID failed after the fixture import → duplicate-note-UUID rejection. Writer now always generates a fresh UUID. Pencil strokes also rendered solid — median-ALPHA opacity fix applied. Awaiting round 3 (roundtrip + B with fresh UUIDs). | PASS (`test_ntb_writer.py`) | maintainer (app) + Claude (builder + uuid fix) | 2026-07-09 |
