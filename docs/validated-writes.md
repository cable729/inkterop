...
| Notability (.ntb) | Notability Mac | ntb-roundtrip + rm-to-notability | Round 1 **FAIL** — "FlatBuffers.FlatbuffersErrors error 0". Structural diff vs the app's buffer: our builder packed fields in the opposite order from the official FlatBuffers builders, didn't dedup vtables, misaligned the root 16-byte struct and zeroed it. All fixed. Round 2: **rm-to-notability PASS**; fixture byte-copy PASS; duplicate-note-UUID rejection found → writer now always generates a fresh UUID. Round 3 (autonomous, **Notability 16** — app updated mid-stream, version-drift flag): **rm-to-notability PASS** (imports, opens, ink renders; fresh-UUID fix confirmed). **ntb-roundtrip silently REJECTED** (no dialog, no note — cold and warm launch). Ruled OUT by controlled variants: point inflation (4× writer/reader flattening bug — found and fixed regardless, 1964→1964 stable now), note-limit (maintainer upgraded to paid), library twin (deleted), title, replayed source creation timestamp, the highlighter stroke. Then the CONTROL (a fresh rm→ntb file, previously importing) ALSO stopped importing — the app's import path itself broke after an appctl Data-restore: Notability's library does NOT live in the container Data dir (survives restores; likely CloudKit-backed), so restoring Data desyncs prefs/import bookkeeping from the library. appctl now refuses to reset Notability. NEXT: retest both files after a genuine app relaunch/sign-in settles, before concluding anything about the file format. `validated` stays False. | PASS (`test_ntb_writer.py`) | Claude (autonomous app loop) | 2026-07-09 |
